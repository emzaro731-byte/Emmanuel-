...
serve(async (req) => {
  ...
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) return jsonResponse({ error: "Payment backend is not configured." }, 500);
  ...
  try {
    ...
    const { data: payment, error: insertError } = await authClient
    ...
    return jsonResponse({
      ...
      instructions: MONIEPOINT_CHECKOUT_URL
        ? "Continue to the secure Moniepoint checkout page. Do not send payment credentials to the app."
        : "A Moniepoint checkout URL has not been configured yet. Use the returned reference when completing a supported manual transfer flow.",
      bank_transfer: MONIEPOINT_ACCOUNT_NUMBER
        ? {
