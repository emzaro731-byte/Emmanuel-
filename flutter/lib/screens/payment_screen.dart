...
class _PaymentScreenState extends State<PaymentScreen>
  ...
  Future<void> _showBankTransfer(String plan, int amount) async {
    setState(() => _loading = true);
    try {
      final data = await PaymentService.instance.startBankTransferPayment(plan: plan);
      if (!mounted) return;
            ...
            if (!_premium)
              Align(
                ...
                child: TextButton.icon(
                  onPressed: _loading ? null : () => _showBankTransfer(name.toLowerCase(), price),
...
class _TransferDialog extends StatefulWidget {
  const _TransferDialog({required this.plan, required this.amount, required this.bank, required this.reference});
  final String plan;
  final int amount;
...
class _TransferDialogState extends State<_TransferDialog> {
  ...
  Future<void> _verify() async {
    ...
    if (reference.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter the transaction reference/session ID.')));
      return;
    }
    setState(() => _verifying = true);
    try {
      final result = await PaymentService.instance.verifyTransaction(reference: reference, plan: widget.plan);
      if (!mounted) return;
      if (result['verified'] == true) {
        Navigator.of(context).pop();
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment verified. Premium access will activate automatically.')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result['message']?.toString() ?? 'Payment could not be verified yet.')));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString())));
    } finally {
  ...
  @override
  Widget build(BuildContext context) {
    final accountName = widget.bank['account_name']?.toString() ?? 'Configure on server';
    final accountNumber = widget.bank['account_number']?.toString() ?? 'Configure on server';
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const Text('Make the transfer, then enter the transaction reference/session ID below. The server will verify the transaction before access is granted.'),
            const SizedBox(height: 12),
            TextField(
      ...
      actions: [
        TextButton(onPressed: _verifying ? null : () => Navigator.of(context).pop(), child: const Text('Close')),
        FilledButton.icon(
          onPressed: _verifying ? null : _verify,
          icon: _verifying ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.verified),
