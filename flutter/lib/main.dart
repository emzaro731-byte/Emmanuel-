...
Future<void> main() async {
  ...
  try {
    ...
    runApp(DestinyApp(
        configError: true,
        message: 'Destiny AI could not initialize its secure backend.\n\n$e'));
  }
}
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Destiny AI',
                    style:
                        TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
                const SizedBox(height: 12),
                Text(message ?? 'Destiny AI could not initialize.',
...
class AuthGate extends StatelessWidget {
  ...
  Widget build(BuildContext context) => StreamBuilder<AuthState>(
        ...
        builder: (_, snap) {
          final signedIn = snap.data?.session != null ||
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Your intelligent AI assistant',
                        textAlign: TextAlign.center,
                    ...
                    TextButton(
                        ...
                        child: Text(register
                            ...
                            : 'New to Destiny AI? Create an account')),
                  ],
                ),
              ),
            ),
...
class _ProfileState extends State<ProfilePage> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      name.text = (row?['display_name'] ??
          ...
          .toString();
      bio.text =
          (row?['bio'] ?? 'Exploring the future with Destiny AI.').toString();
      setState(() {});
    } catch (_) {}
  }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      await Supabase.instance.client.from('destiny_profiles').upsert({
        'id': user.id,
  ...
  Widget build(BuildContext context) {
    final user = Supabase.instance.client.auth.currentUser;
    final display = name.text.isEmpty
        ? (user?.email?.split('@').first ?? 'Destiny User')
        : name.text;
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Center(
              child: CircleAvatar(
                  radius: 42,
                  backgroundColor: gold.withValues(alpha: .14),
                  child: Text(display.isEmpty ? 'D' : display[0].toUpperCase(),
          ...
          if (editing) ...[
            ...
            Card(
                ...
                child: Padding(
                    ...
                    child: Text(
                        bio.text.isEmpty ? 'Welcome to Destiny AI.' : bio.text,
                        style: const TextStyle(color: muted, height: 1.45)))),
            const SizedBox(height: 14),
            _InfoCard(
                icon: Icons.workspace_premium_outlined,
                title: 'Destiny AI Free',
            ...
            const _InfoCard(
                ...
                title: 'AI Studio',
                subtitle: 'Create images, videos and music with AI.',
                action: 'Open'),
            const _InfoCard(
                icon: Icons.cloud_outlined,
                title: 'Cloud history',
                subtitle: 'Your conversations can sync with your account.',
                action: 'Enabled'),
          ],
        ],
      ),
        ...
        body: ListView(
          ...
          children: [
            ...
            Card(
                ...
                child: Column(children: [
                  SwitchListTile(
                      ...
                      subtitle: const Text('Receive useful Destiny AI updates'),
                      value: notifications,
                      onChanged: (v) {
                  ...
                  SwitchListTile(
                      ...
                      subtitle:
                          const Text('Keep a local backup of conversations'),
                      value: saveHistory,
                      onChanged: (v) {
                  ...
                  SwitchListTile(
                      title: const Text('Compact interface'),
                      subtitle: const Text(
                          'Use tighter spacing in supported screens'),
                      value: compact,
                      onChanged: (v) {
                        setState(() => compact = v);
                        _set('compact_mode', v);
            ...
            Card(
                ...
                child: Column(children: [
                  const ListTile(
                      leading: Icon(Icons.psychology_outlined, color: gold),
                      title: Text('AI modes'),
                      subtitle: Text('Chat, Code, Study, Write and Creative')),
                  const ListTile(
                      leading: Icon(Icons.auto_awesome, color: gold),
                      title: Text('AI Studio'),
                      subtitle: Text('Image, video and music generation')),
                  const ListTile(
                      leading: Icon(Icons.security_outlined, color: gold),
                      title: Text('Privacy & security'),
                      subtitle: Text('Provider credentials stay server-side.')),
                  const ListTile(
                      leading: Icon(Icons.info_outline, color: gold),
                      title: Text('About Destiny AI'),
            ...
            const Center(
                child: Text('Destiny AI • Built for the future',
                    style: TextStyle(color: muted))),
          ],
        ),
      );
