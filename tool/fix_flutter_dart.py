...
def repair_widgets(text: str) -> str:
  ...
  Widget _messagesView() {
    if (_messages.isEmpty) return ListView(padding: const EdgeInsets.fromLTRB(20, 44, 20, 24), children: [
      const _HeroOrb(), const SizedBox(height: 22),
      ...
      const Text('Chat, code, study, write and create\nwith live AI responses.', textAlign: TextAlign.center, style: TextStyle(color: _muted, height: 1.5)),
      const SizedBox(height: 25),
      _suggestion('Explain something to me simply'), _suggestion('Help me write a professional message'),
      _suggestion('Build a modern app idea with me'), _suggestion('Create an image or video concept'),
    ]);
    return ListView.builder(
      controller: _scroll,
      padding: const EdgeInsets.fromLTRB(14, 12, 14, 18),
      itemCount: _messages.length,
      itemBuilder: (_, i) => _Bubble(message: _messages[i], streaming: _messages[i].id == _streamingId, onCopy: _copyMessage, onShare: _shareMessage, onSpeak: _speakMessage),
    );
  }
...
def add_voice_and_actions(text: str) -> str:
  ...
  bool _speaking = false;"""
    if 'final SpeechToText _speech' not in text and state in text:
        text = text.replace(state, state_new, 1)

    # Methods before _send.
    ...
    if (!available) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Voice input is not available on this device.')));
      return;
    }
...
'''
    if 'Future<void> _toggleListening()' not in text and anchor in text:
        text = text.replace(anchor, methods + anchor, 1)

    # Make sure the bubble receives action callbacks even if the widget section was not repaired.
    old = "_Bubble(message: _messages[i], streaming: _messages[i].id == _streamingId)"
    new = "_Bubble(message: _messages[i], streaming: _messages[i].id == _streamingId, onCopy: _copyMessage, onShare: _shareMessage, onSpeak: _speakMessage)"
    text = text.replace(old, new)

    # Replace bubble class with action buttons and parser-safe conditionals.
    ...
    return Align(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            user ? const SizedBox.shrink() : const _AIAvatar(),
            user ? const SizedBox.shrink() : const SizedBox(width: 7),
            Flexible(child: Column(crossAxisAlignment: user ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
              Container(
                ...
                decoration: BoxDecoration(color: user ? _purple.withOpacity(.21) : Colors.white.withOpacity(.065), borderRadius: BorderRadius.only(topLeft: const Radius.circular(20), topRight: const Radius.circular(20), bottomLeft: Radius.circular(user ? 20 : 5), bottomRight: Radius.circular(user ? 5 : 20)), border: Border.all(color: Colors.white.withOpacity(.10))),
                child: RichText(text: TextSpan(style: const TextStyle(color: _text, fontSize: 15, height: 1.5), children: [TextSpan(text: message.text), streaming ? const TextSpan(text: ' ▌', style: TextStyle(color: _purple, fontWeight: FontWeight.w900)) : const TextSpan(text: '')]),
              ),
              if (!user && !streaming && message.text.trim().isNotEmpty)
                Padding(padding: const EdgeInsets.only(top: 5, left: 3), child: Row(mainAxisSize: MainAxisSize.min, children: [
...
for path in FILES:
    ...
    updated = add_voice_and_actions(updated)
    if updated != original:
