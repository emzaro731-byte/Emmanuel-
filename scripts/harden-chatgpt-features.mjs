...
const replaceOnce = (from, to, label) => {
  if (!app.includes(from)) throw new Error(`Missing feature hardening anchor: ${label}`);
...
`  // CHATGPT_FEATURES_HARDENED_V1\n  const createProject = async () => {\n    const value = `Project ${projects.length + 1}`;\n    const next = [...projects, value];\n    setProjects(next);\n    setSelectedProject(value);\n    await AsyncStorage.setItem("destiny_ai_projects_v1", JSON.stringify(next));\n    Alert.alert("Project created", `${value} is ready. You can rename it later from project settings.`);\n  };`,
